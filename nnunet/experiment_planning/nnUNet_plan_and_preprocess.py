...
def main():
    ...
    parser.add_argument("-target_task", "--target_task", type=str, help="Target Preprocessing Task Name")
    parser.add_argument("-tp", "--target_patch_size", nargs="+", help="List of integers that shows patch_size")
    parser.add_argument("-tb", "--target_batch_size", type=str, help="List of integers that shows patch_size")
    ...
    if args.target_patch_size is not None:
        target_patch_size = [int(x) for x in args.target_patch_size]
    else:
    ...
    raw_task = convert_id_to_task_name(int(task_id))

    if target_task is None:
        target_task = raw_task
        
    if args.verify_dataset_integrity:
        verify_dataset_integrity(join(nnUNet_raw_data, raw_task))

    crop(raw_task, False, tf)
    #crop(raw_task, target_task, False, tf)  # It looks 하나의 CROP만 있어도 될 것 같음, 하지만 안되면 수정필요
    ...
    #for t in tasks:
    print(f"\n\n\n Raw: {task_id} Target: {target_task}")
    cropped_out_dir = os.path.join(nnUNet_cropped_data, raw_task)
    preprocessing_output_dir_this_task = os.path.join(preprocessing_output_dir, target_task)
    ...
    shutil.copy(join(nnUNet_raw_data, raw_task, "dataset.json"), preprocessing_output_dir_this_task)
    ...
    if planner_3d is not None:
        if args.overwrite_plans is not None:
            ...
            exp_planner = planner_3d(cropped_out_dir, preprocessing_output_dir_this_task, args.overwrite_plans,
                                        args.overwrite_plans_identifier, target_patch_size, target_batch_size)
        else:
            exp_planner = planner_3d(cropped_out_dir, preprocessing_output_dir_this_task, target_patch_size, target_batch_size)
