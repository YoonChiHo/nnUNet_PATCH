    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.patch_select is not None:
                ...
                # Negative PATCH
                cnt=0
                while len(negative_selected) < self.patch_select[1]:
                    bbox_x_lb = np.random.randint(lb_x, ub_x + 1)
                    bbox_y_lb = np.random.randint(lb_y, ub_y + 1)
                    bbox_z_lb = np.random.randint(lb_z, ub_z + 1)
                    cnt = cnt + 1
                    if cnt > 100:
                        print(f'ERROR: n repeated {cnt} {len(negative_selected)}/{self.patch_select[1]}')
                ...
                # Positive PATCH
                cnt=0
                while len(positive_selected) < self.patch_select[0]:
                    cnt = cnt + 1
                    if cnt > 100:
                        print(f'ERROR: p repeated {cnt} {len(positive_selected)}/{self.patch_select[0]}')
